...
export function DashboardToolbar({
  ...
  return (
    <header className="h-14 flex items-center justify-between px-4 md:px-6 border-b border-white/[0.06] flex-shrink-0 bg-gs-body relative">
      <div className="flex items-center gap-2 md:gap-4 min-w-0">
        <QueryTitle title={queryTitle} />

        <div className="hidden md:block h-4 w-px bg-white/[0.06] mx-1" />
        ...
        <div className="flex items-center gap-1 text-gs-text-muted">
          ...
          <button className="hidden md:block p-1.5 hover:bg-white/[0.06] rounded-md hover:text-gs-text-main transition-colors">
        ...
        <button onClick={onEnrichClick} className="flex items-center gap-2 text-xs font-medium text-gs-text-muted hover:text-white px-2 md:px-3 py-1.5 hover:bg-white/[0.06] rounded-md border border-transparent hover:border-white/[0.06] transition-all">
          <Icon icon="lucide:sparkles" className="w-3.5 h-3.5" />
          <span className="hidden md:inline">Enrich Data</span>
        </button>
      </div>
